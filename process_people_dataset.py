...
CSV_COLUMNS = [
    ...
    "upscale_model", "upscale_scale", "status", "failures_json",
...
def save_crops(
    ...
    with Image.open(source) as opened:
        ...
        for index, detection in enumerate(detections, start=1):
            output = crops_dir / f"{source.stem}_person_{index:02d}.jpg"
...
def main() -> int:
    ...
    for number, source in enumerate(files, start=1):
        ...
        if not args.skip_detection and not failures:
            try:
                ...
                if cached and cached.get("source_signature") == source_signature:
                    ...
                    cached = {
                        "filename": source.name, "source_signature": source_signature,
                        "config_id": config_id,
                        "detections": [asdict(item) for item in detections],
                    }
                    append_detection_cache(cache_path, cached)
                    cache[source.name] = cached
                crop_outputs = save_crops(source, detections, crops_dir, args.crop_size, args.overwrite)
            except Exception as error:
        ...
        rows.append({
            ...
            "crop_paths": crop_paths, "crop_paths_json": json.dumps(crop_paths),
            "detection_confidences": confidences,
            "detection_confidences_json": json.dumps(confidences),
            "detection_boxes": boxes, "detection_boxes_json": json.dumps(boxes),
            ...
            "upscale_scale": args.upscale_scale, "status": status,
            "failures": failures, "failures_json": json.dumps(failures),
    ...
    print(
        ...
        f"{sum(int(row['detected_person_count']) for row in rows)} person crops, "
        f"{sum(row['status'] in {'partial', 'failed'} for row in rows)} partial/failed, "
        f"{sum(row['status'] == 'awaiting_upscale' for row in rows)} awaiting upscale.",
        flush=True,
    )
    return 0
